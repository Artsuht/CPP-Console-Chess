...
void Queen::MovePiece(std::vector<ChessPieces>& piece, ChessBoard& chess_board, int index, int move_dist, bool diagonal, int left_right, int up_down)
{
	if (!piece[index].captured && InBounds(chess_board, piece[index].piece_x, piece[index].piece_y))
	{
		...
		{
		  if(InBounds(chess_board, piece[index].piece_x + (Piece_Info::SQUARES * move_dist), piece[index].piece_y)
			  && !IsFriendly(chess_board, piece[index].piece_x + (Piece_Info::SQUARES * move_dist), piece[index].piece_y))
			{
		...
		{
		  if (InBounds(chess_board, piece[index].piece_x, piece[index].piece_y) + (Piece_Info::SQUARES * move_dist)
			  && !IsFriendly(chess_board, piece[index].piece_x, piece[index].piece_y + (Piece_Info::SQUARES * move_dist)))
		  {
			  chess_board.friendly_squares[piece[index].on_square] = false;
		...
		{
			if (InBounds(chess_board, piece[index].piece_x + (Piece_Info::SQUARES * move_dist), piece[index].piece_y) + (Piece_Info::SQUARES * move_dist)
				&& !IsFriendly(chess_board, piece[index].piece_x + (Piece_Info::SQUARES * move_dist), piece[index].piece_y + (Piece_Info::SQUARES * move_dist)))
			{
				chess_board.friendly_squares[piece[index].on_square] = false;
